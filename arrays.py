...
class SQArray(QuaternionPropertiesMixin, QuaternionConvertersMixin, np.ndarray):
    """Subclass of numpy arrays interpreted as split quaternions.

    This class encapsulates ths split quaternion algebra, with numpy's "ufunc"s
    overridden by methods from the algebra module. Standard algebraic operations can be
    performed naturally — as in `q1+q2`, `q1*q2`, etc, using the standard numpy broadcasting.
    Numpy functions can also be used as expected — as in `np.exp(q)`, `np.sqrt(q)`, etc.
    ...
    as a split quaternion array:
    import split_quaternions as sq
        q1 = sq.array([1, 2, 3, 4])  # explicit array
        q2 = sq.array(np.random.random(size=(10, 4))  # re-interpret ndarray
    ...
    extra properties that are particularly useful for quaternions, including
      ...
      * Methods to extract and/or set components
        ...
        * scalar/vector
      * Parameter methods
        * abs/modulus (square-root of sum of squares of components)
    ...
    def __array_finalize__(self, obj):
        if self.shape[-1] != 4:
            raise ValueError(
                ...
                "last dimension should have size 4 to represent a split quaternion.\n"
                "If you are trying to slice the split quaternions, you should append `.ndarray` before slicing.\n"
                ...
                "Also note that split quaternions have attributes like `q.w`, `q.x`, `q.y`, and `q.z` to return\n"
            )

    def __array_function__(self, func, types, args, kwargs):
        if func == np.angle:
            output = np.zeros(args[0].shape[:-1], dtype=float)
    ...
    def __array_ufunc__(self, ufunc, method, *args, **kwargs):
        ...
        if ufunc in [np.add, np.subtract, np.multiply, np.divide, np.true_divide]:
            ...
            if this_type(args[0]) and this_type(args[1]):
                ...
                if isinstance(result, tuple):
                    result = result[0]
                result = result.view(np.ndarray)  # not sure of the use of this line of code... Seems useless
        ...
        # float64[4](float64[4])
            ...
            np.exp, np.sqrt, np.square, np.reciprocal]:
            if this_type(args[0]):
                a1 = args[0]
                result = out or np.zeros(a1.shape, dtype=float)
                if isinstance(result, tuple):
                    result = result[0]
                if isinstance(result, type(self)):
                    result = result.view(np.ndarray)
        ...
        # float64(float64[4])
            ...
            if this_type(args[0]):
                a1 = args[0]
                result = out or np.zeros(a1.shape[:-1], dtype=float)
                if isinstance(result, tuple):
                    result = result[0]
                if isinstance(result, type(self)):
                    result = result.view(np.ndarray)
        ...
        # bool(float64[4])
            ...
            if this_type(args[0]):
                ...
                return NotImplemented
        elif ufunc in [np.matmul]:
            if this_type(args[0]) or this_type(args[1]):
                a1, a2 = args[:2]
                if not this_type(a1):
                    a1 = type(self).from_scalar_part(a1)
                if not this_type(a2):
                    a2 = type(self).from_scalar_part(a2)
                shape1 = a1.shape
                shape2 = a2.shape
                shape = (shape1[0], shape2[1], 4)
                result = out or np.zeros(shape, dtype=dtype)
                if isinstance(result, tuple):
                    result = result[0]
                result = result.view(np.ndarray)
    ...
    def __repr__(self):
        return 'split_quaternion.' + repr(self.ndarray)

    def __str__(self):
        return str(self.ndarray)

    # overide native transpose
    @property
    def T(self):
        if len(self.shape) == 3:
